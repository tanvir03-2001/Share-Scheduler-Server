 ...
 */
export const scheduleImmediate = async (req: Request, res: Response): Promise<void> => {
    try {
 ...
 */
export const getUpcomingPosts = async (req: Request, res: Response): Promise<void> => {
    try {
        const userId = (req as any).jwtUser?.id;
        if (!userId) {
            sendResponse(res, 401, false, 'User not authenticated');
 ...
 */
export const getScheduledHistory = async (req: Request, res: Response): Promise<void> => {
    try {
        const userId = (req as any).jwtUser?.id;
        if (!userId) {
            sendResponse(res, 401, false, 'User not authenticated');
